...
public class CritterProgram extends GraphicsProgram {
    ...
    private GPoint randLocationInRange(GObject g) {
        double maxRange = 500.0;
        GPoint p = new GPoint(g.getLocation());
        p.translate(
                rand.nextDouble() * (2.0 *maxRange) - maxRange ,
                rand.nextDouble() * (2.0 * maxRange) - maxRange);
        p.setLocation(Math.max(0.0, Math.min(p.getX(), getWidth() - g.getWidth())), Math.max(0.0, Math.min(p.getY(), getHeight() - g.getHeight())));
        return p;
    }
}
